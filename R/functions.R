...
left_join_strict <- function(left_df, right_df, by = NULL, ...) {
  ...
  if (nrow(unmatched) > 0) {
    stop("Error: Some rows in the left dataset do not have matching keys in the right dataset.")
  }
...
get_co2_ets <- function(GCAM_version = 'v7.0') {
  ...
  var_fun_map <- get(paste('var_fun_map',GCAM_version,sep='_'), envir = asNamespace("gcamreport"))
  queryItem1 <- var_fun_map[var_fun_map$name == "co2_ets_byreg", "queries"][[1]][1]
  queryItem2 <- var_fun_map[var_fun_map$name == "co2_ets_byreg", "queries"][[1]][2]
  ...
  co2_ets_bysec <<-
    tibble::as_tibble(rgcam::getQuery(prj, queryItem2)) %>%
    dplyr::filter(ghg == "CO2_ETS") %>%
...
get_primary_energy <- function(GCAM_version = "v7.0") {
  ...
  primary_energy_clean <<-
    ...
    left_join_strict(filter_variables(get(paste('primary_energy_map',GCAM_version,sep='_'), envir = asNamespace("gcamreport")), "primary_energy_clean"), by = c("fuel"), multiple = "all") %>%
    dplyr::filter(var != 'NoReported') %>%
    dplyr::filter(!is.na(var)) %>%
    dplyr::mutate(value = value * unit_conv) %>%
    dplyr::group_by(scenario, region, year, var) %>%
...
get_elec_gen_tech <- function(GCAM_version = "v7.0") {
  ...
  elec_gen_tech_clean <<-
    ...
    left_join_strict(filter_variables(get(paste('elec_gen_map',GCAM_version,sep='_'), envir = asNamespace("gcamreport")), "elec_gen_tech_clean"), by = c("output", "subsector", "technology"), multiple = "all") %>%
    dplyr::filter(!is.na(var)) %>%
    dplyr::mutate(value = value * unit_conv) %>%
    dplyr::group_by(scenario, region, year, var) %>%
    dplyr::summarise(value = sum(value, na.rm = T)) %>%
...
get_se_gen_tech <- function(GCAM_version = "v7.0") {
  ...
  se_gen_tech_clean <<-
    ...
    left_join_strict(filter_variables(get(paste('se_gen_map',GCAM_version,sep='_'), envir = asNamespace("gcamreport")), "se_gen_tech_clean"), by = c("sector", "subsector", "technology"), multiple = "all") %>%
    dplyr::filter(!is.na(var)) %>%
    dplyr::mutate(value = value * unit_conv) %>%
    dplyr::group_by(scenario, region, year, var) %>%
    dplyr::summarise(value = sum(value, na.rm = T)) %>%
...
get_fe_transportation_tmp <- function(GCAM_version = "v7.0") {
  ...
  fe_transportation <<-
    ...
    left_join_strict(filter_variables(get(paste('transport_final_en_map',GCAM_version,sep='_'), envir = asNamespace("gcamreport")), "fe_transportation"),
                     by = c("sector", "input", "mode"), multiple = "all") %>%
    dplyr::filter(var != 'NoReported') %>%
    dplyr::filter(!is.na(var)) %>%
    dplyr::mutate(value = value * unit_conv) %>%
    dplyr::group_by(scenario, region, year, var) %>%
...
get_energy_service_transportation <- function(GCAM_version = "v7.0") {
  ...
  energy_service_transportation_clean <<-
    ...
    left_join_strict(filter_variables(get(paste('transport_en_service',GCAM_version,sep='_'), envir = asNamespace("gcamreport")), "energy_service_transportation_clean"), by = c("sector", "mode"), multiple = "all") %>%
    dplyr::filter(var != 'NoReported') %>%
    dplyr::filter(!is.na(var)) %>%
    dplyr::mutate(value = value * unit_conv) %>%
    dplyr::group_by(scenario, region, year, var) %>%
...
get_industry_production <- function(GCAM_version = "v7.0") {
  ...
  industry_production_clean <<-
    ...
    left_join_strict(filter_variables(get(paste('production_map',GCAM_version,sep='_'), envir = asNamespace("gcamreport")), "industry_production_clean"), by = c("sector")) %>%
    dplyr::filter(var != 'NoReported') %>%
...
get_iron_steel_imports <- function(GCAM_version = "v7.0") {
  ...
  if ("regional iron and steel sources" %in% rgcam::listQueries(prj)) {
    iron_steel_imports <<-
    ...
    iron_steel_imports <<-
      tidyr::tibble(var = c('Import|Industry|Steel|Value')) %>%
      tidyr::expand_grid(tibble::tibble(scenario = scenarios.global)) %>%
      tidyr::expand_grid(tibble::tibble(year = available_reporting_years[available_reporting_years <= final_year.global])) %>%
      tidyr::expand_grid(tibble::tibble(region = c(desired_regions, "Global"))) %>%
...
get_iron_steel_exports <- function(GCAM_version = "v7.0") {
  ...
  if ("traded iron and steel" %in% rgcam::listQueries(prj)) {
    iron_steel_exports <<-
    ...
    iron_steel_exports <<-
      tidyr::tibble(var = c('Export|Industry|Steel|Value')) %>%
      tidyr::expand_grid(tibble::tibble(scenario = scenarios.global)) %>%
      tidyr::expand_grid(tibble::tibble(year = available_reporting_years[available_reporting_years <= final_year.global])) %>%
      tidyr::expand_grid(tibble::tibble(region = c(desired_regions, "Global"))) %>%
      dplyr::mutate(value = 0) %>%
...
get_co2_price_share <- function(GCAM_version = "v7.0") {
  ...
  co2_price_share_byreg_tmp <- co2_clean %>%
    dplyr::filter(
  ...
  # compute the CO2 ETS vs CO2 regional emission shares
  co2_price_share_byreg_share_CO2_ETS <- co2_price_share_byreg_tmp %>%
  ...
  if (!("CO2_ETS" %in% colnames(co2_price_share_byreg_share_CO2_ETS))) {
    co2_price_share_byreg_share_CO2_ETS <- co2_price_share_byreg_share_CO2_ETS %>%
  ...
  # compute the regional vs World CO2 emission shares
  co2_price_share_byreg_share_CO2_world <- co2_price_share_byreg_tmp %>%
    dplyr::group_by(scenario, var, year) %>%
  ...
  co2_price_share_byreg <<- merge(
    ...
    co2_price_share_byreg_share_CO2_world,
    by = c('scenario','region','year')
  )

  co2_price_share_bysec_tmp <<- co2_clean %>%
...
get_prices_subsector <- function(GCAM_version = "v7.0") {
  ...
  if (sum(unique(rgcam::getQuery(prj, "prices by sector")$sector) %in% unique(energy_prices_map$sector)) != 17) {
    handle_warning(mapping_name1 = 'energy_prices_map', query_name = 'prices by sector')
  }

  if (sum(unique(rgcam::getQuery(prj, "costs by subsector")$sector) %in% unique(energy_prices_map$sector)) != 17) {
...
get_cf_iea_tmp <- function(GCAM_version = "v7.0") {
  ...
  cf_iea <<-
    ...
    left_join_strict(filter_variables(get(paste('capacity_map',GCAM_version,sep='_'), envir = asNamespace("gcamreport")), "cf_iea"), by = "var", multiple = "all") %>%
    dplyr::filter(var != 'NoReported') %>%
...
get_elec_capacity_tot <- function(GCAM_version = "v7.0") {
  ...
  elec_capacity_tot_clean <<- suppressWarnings(
    rgcam::getQuery(prj, "elec gen by gen tech and cooling tech and vintage") %>%
      ...
      dplyr::ungroup() %>%
      left_join_strict(filter_variables(get(paste('capacity_map',GCAM_version,sep='_'), envir = asNamespace("gcamreport")), "elec_capacity_tot_clean") %>% dplyr::select(-output), by = c("technology"), multiple = "all") %>%
      dplyr::filter(!is.na(var)) %>%
      dplyr::mutate(
        value = value * unit_conv,
...
get_elec_capacity_add <- function(GCAM_version = "v7.0") {
  ...
  elec_capacity_add_clean <<-
    elec_capacity_add %>%
    left_join_strict(get(paste('capacity_map',GCAM_version,sep='_'), envir = asNamespace("gcamreport")) %>% dplyr::select(-output), by = c("technology"), multiple = "all") %>%
    dplyr::filter(var != 'NoReported') %>%
...
get_elec_capital <- function(GCAM_version = "v7.0") {
  ...
  elec_capital <-
    ...
    dplyr::filter(sector != 'NoReported') %>%
    ...
    dplyr::mutate(value = capital.overnight *
                    get(paste('convert',GCAM_version,sep='_'), envir = asNamespace("gcamreport"))[['conv_75USD_10USD']]) %>%
    left_join_strict(filter_variables(get(paste('elec_gen_map',GCAM_version,sep='_'), envir = asNamespace("gcamreport")), "elec_capital") %>% dplyr::select(-output), by = c("subsector", "technology"), multiple = "all")
...
get_elec_investment <- function(GCAM_version = "v7.0") {
  ...
  elec_gen_map <- filter_variables(get(paste('elec_gen_map',GCAM_version,sep='_'), envir = asNamespace("gcamreport")), "elec_investment_clean") %>% dplyr::select(-output)
  ...
  elec_investment_clean <<-
    ...
    dplyr::mutate(value = GW * capital.overnight / 1000 *
                    get(paste('convert',GCAM_version,sep='_'), envir = asNamespace("gcamreport"))[['conv_75USD_10USD']]) %>%
    left_join_strict(elec_gen_map, by = c("technology", "subsector"), multiple = "all") %>%
    dplyr::filter(!is.na(var)) %>%
    dplyr::mutate(
      value = value * unit_conv,
