...
# capital updates
capital_gcam <- read.csv(file.path(rawDataFolder, "inst/extdata/mappings", "L223.GlobalIntTechCapital_elec.csv"),
  skip = 2, na = "",
  fileEncoding = "UTF-8-BOM", stringsAsFactors = FALSE
) %>%
  dplyr::rename(technology = intermittent.technology) %>%
  dplyr::bind_rows(read.csv(file.path(rawDataFolder, "inst/extdata/mappings", "L223.GlobalTechCapital_elec.csv"),
    skip = 2, na = "",
    stringsAsFactors = FALSE
  )) %>%
