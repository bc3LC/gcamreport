...
# emissions maps
co2_sector_map <- read.csv(paste0(rawDataFolder, "/inst/extdata/mappings", "/CO2_sector_map.csv"), skip = 1, na = "") %>% gather_map()
use_data(co2_sector_map, overwrite=T)

co2_tech_map <- read.csv(paste0(rawDataFolder, "/inst/extdata/mappings", "/CO2_tech_map.csv"), skip = 1, na = "") %>% gather_map()
use_data(co2_tech_map, overwrite=T)

kyoto_sector_map <- read.csv(paste0(rawDataFolder, "/inst/extdata/mappings", "/Kyotogas_sector.csv"), skip = 1, na = "") %>% gather_map()
use_data(kyoto_sector_map, overwrite=T)

nonco2_emis_sector_map <- read.csv(paste0(rawDataFolder, "/inst/extdata/mappings", "/nonCO2_emissions_sector_map.csv"), skip = 1, na = "") %>% gather_map()
use_data(nonco2_emis_sector_map, overwrite=T)

nonco2_emis_resource_map <- read.csv(paste0(rawDataFolder, "/inst/extdata/mappings", "/nonCO2_emissions_resource_map.csv"), skip = 1, na = "") %>% gather_map()
use_data(nonco2_emis_resource_map, overwrite=T)

carbon_seq_tech_map <- read.csv(paste0(rawDataFolder, "/inst/extdata/mappings", "/carbon_seq_tech_map.csv"), skip = 1, na = "") %>% gather_map()
use_data(carbon_seq_tech_map, overwrite=T)


# ag maps
ag_demand_map <- read.csv(paste0(rawDataFolder, "/inst/extdata/mappings", "/ag_demand_map.csv"), skip = 1) %>% gather_map()
use_data(ag_demand_map, overwrite=T)

ag_prices_map <- read.csv(paste0(rawDataFolder, "/inst/extdata/mappings", "/ag_prices_map.csv"), skip = 1) %>% gather_map()
use_data(ag_prices_map, overwrite=T)

land_use_map <- read.csv(paste0(rawDataFolder, "/inst/extdata/mappings", "/land_use_map.csv"), skip = 1) %>% gather_map()
use_data(land_use_map, overwrite=T)


# primary, secondary, final energy maps
primary_energy_map <- read.csv(paste0(rawDataFolder, "/inst/extdata/mappings", "/en_primary_map.csv"), skip = 1) %>% gather_map()
use_data(primary_energy_map, overwrite=T)

production_map <- read.csv(paste0(rawDataFolder, "/inst/extdata/mappings","/production_map.csv"), skip = 1) %>% gather_map()
...
se_gen_map <- read.csv(paste0(rawDataFolder, "/inst/extdata/mappings", "/secondary_energy_gen_map.csv"), skip = 1) %>% gather_map()
use_data(se_gen_map, overwrite=T)

final_energy_map <- read.csv(paste0(rawDataFolder, "/inst/extdata/mappings", "/final_energy_map.csv"), skip = 1) %>% gather_map()
use_data(final_energy_map, overwrite=T)

transport_final_en_map <- read.csv(paste0(rawDataFolder, "/inst/extdata/mappings", "/transport_final_en_map.csv"), skip = 1, na = "") %>% gather_map()
use_data(transport_final_en_map, overwrite=T)

energy_prices_map <- read.csv(paste0(rawDataFolder, "/inst/extdata/mappings", "/energy_prices_map.csv"), skip = 1, na = "") %>% gather_map()
use_data(energy_prices_map, overwrite=T)


#Energy Service maps
transport_en_service <- read.csv(paste0(rawDataFolder, "/inst/extdata/mappings", "/energy_service_transportation.csv"), skip = 1) %>% gather_map()
use_data(transport_en_service, overwrite=T)

buildings_en_service <- read.csv(paste0(rawDataFolder, "/inst/extdata/mappings", "/energy_service_buildings.csv"), skip = 1) %>% gather_map()
...
investment <- read.csv(paste0(rawDataFolder, "/inst/extdata/mappings", "/investment.csv"), na = "") %>%
...
iea_capacity <- read.csv(paste0(rawDataFolder, "/inst/extdata/mappings", "/IEAWEO2019_Capacity.csv"))
use_data(iea_capacity, overwrite=T)

CO2_market <- read.csv(paste0(rawDataFolder, "/inst/extdata/mappings", "/CO2market_new.csv"), skip = 1)
