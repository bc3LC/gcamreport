...
final_energy_input <- c('Final Energy',
                        ...
                        'Final Energy|Solids|Biomass|Modern',
                        'Final Energy|Solids|Biomass|Traditional',
                        'Final Energy|Solids|Coal')
use_data(final_energy_input, overwrite = T)

final_energy_sector <- c('Final Energy\\|Carbon Removal',
                         'Final Energy\\|Carbon Removal\\|Direct Air Capture',
                         'Final Energy\\|Carbon Removal\\|Direct Air Capture\\|Electricity',
                         ...
                         'Final Energy\\|Other',
                         'Final Energy\\|Other\\|Electricity',
                         'Final Energy\\|Other\\|Gases',
                         'Final Energy\\|Other\\|Liquids',
...
use_data(final_energy_sector, overwrite = T)

final_energy_industry_sectors1 <- c('Final Energy\\|Industry\\|Cement',
...
use_data(final_energy_industry_sectors1, overwrite = T)

final_energy_industry_sectors2 <- c('Final Energy\\|Industry\\|Chemicals\\|Ammonia')
use_data(final_energy_industry_sectors2, overwrite = T)

final_energy_residcomm_sectors <- c('Final Energy\\|Residential and Commercial\\|Commercial',
                                    'Final Energy\\|Residential and Commercial\\|Residential')
use_data(final_energy_residcomm_sectors, overwrite = T)

final_energy_residcomm_space <- c('Final Energy\\|Residential and Commercial\\|Cooling',
                                  'Final Energy\\|Residential and Commercial\\|Heating')
use_data(final_energy_residcomm_space, overwrite = T)

final_energy_transport_mode <- c('Final Energy\\|Transportation\\|Freight',
                                 'Final Energy\\|Transportation\\|Passenger')
use_data(final_energy_transport_mode, overwrite = T)

final_energy_transport_sector <- c('Final Energy\\|Transportation\\|Aviation',
                                   'Final Energy\\|Transportation\\|Maritime',
                                   'Final Energy\\|Transportation\\|Road',
                                   'Final Energy\\|Transportation\\|Rail')
use_data(final_energy_transport_sector, overwrite = T)


en_blocks <- read.csv(file.path(rawDataFolder, "inst/extdata/mappings/GCAM7.1", "en_blocks.csv"),
                      skip = 1,
                      stringsAsFactors = FALSE,
