...
reg_cont_v7.1 <- readr::read_csv(file.path(rawDataFolder, "inst/extdata/mappings/GCAM7.1", "regions_continents_map.csv"),
                          comment = "#"
)
...
co2_tech_map_v7.1 <- readr::read_csv(file.path(rawDataFolder, "inst/extdata/mappings/GCAM7.1", "CO2_tech_map.csv"),
                              comment = "#", na = ""
) %>% gather_map()
...
kyoto_sector_map_v7.1 <- readr::read_csv(file.path(rawDataFolder, "inst/extdata/mappings/GCAM7.1", "kyotogas_sector.csv"),
                                  comment = "#", na = ""
) %>% gather_map()
use_data(kyoto_sector_map_v7.1, overwrite = T)

nonco2_emis_sector_map_v7.1 <- readr::read_csv(file.path(rawDataFolder, "inst/extdata/mappings/GCAM7.1", "nonCO2_emissions_sector_map.csv"),
                                        comment = "#", na = ""
) %>% gather_map()
use_data(nonco2_emis_sector_map_v7.1, overwrite = T)

nonco2_emis_resource_map_v7.1 <- readr::read_csv(file.path(rawDataFolder, "inst/extdata/mappings/GCAM7.1", "nonCO2_emissions_resource_map.csv"),
                                          comment = "#", na = ""
) %>% gather_map()
use_data(nonco2_emis_resource_map_v7.1, overwrite = T)

carbon_seq_tech_map_v7.1 <- readr::read_csv(file.path(rawDataFolder, "inst/extdata/mappings/GCAM7.1", "carbon_seq_tech_map.csv"),
                                     comment = "#", na = ""
) %>% gather_map()
...
ag_price_map_v7.1 <- readr::read_csv(file.path(rawDataFolder, "inst/extdata/mappings/GCAM7.1", "ag_price_map.csv"),
                               comment = "#"
) %>% gather_map()
use_data(ag_price_map_v7.1, overwrite = T)

ag_production_map_v7.1 <- readr::read_csv(file.path(rawDataFolder, "inst/extdata/mappings/GCAM7.1", "ag_production_map.csv"),
                               comment = "#"
) %>% gather_map()
use_data(ag_production_map_v7.1, overwrite = T)

ag_demand_price_map_v7.1 <- readr::read_csv(file.path(rawDataFolder, "inst/extdata/mappings/GCAM7.1", "ag_demand_price_map.csv"),
                               comment = "#"
)
use_data(ag_demand_price_map_v7.1, overwrite = T)

trade_ag_v7.1 <- readr::read_csv(file.path(rawDataFolder, "inst/extdata/mappings/GCAM7.1", "trade_ag.csv"), comment = "#") %>% gather_map()
use_data(trade_ag_v7.1, overwrite = T)

land_use_map_v7.1 <- readr::read_csv(file.path(rawDataFolder, "inst/extdata/mappings/GCAM7.1", "land_use_map.csv"),
                              comment = "#"
) %>% gather_map()
...
food_intake_map_v7.1 <- readr::read_csv(file.path(rawDataFolder, "inst/extdata/mappings/GCAM7.1", "food_intake_map.csv"),
                              comment = "#"
) %>% gather_map()
use_data(food_intake_map_v7.1, overwrite = T)

food_items_map_v7.1 <- readr::read_csv(file.path(rawDataFolder, "inst/extdata/mappings/GCAM7.1", "food_items_map.csv"),
                              comment = "#"
)
...
use_data(L100.AgMIP_FoodWaste_Share_Pathway_SSP_v7.1, overwrite = T)

# wood fuel - industrial roundwood
WoodFuel_IndRoundwood_ratio_v7.1 <- readr::read_csv(file.path(rawDataFolder, "inst/extdata/mappings/GCAM7.1", "FAO_GCAM_Ratio_WoodFuel_IndRoundwood_R_Yh.csv"),
                                                    comment = "#"
)
use_data(WoodFuel_IndRoundwood_ratio_v7.1, overwrite = T)

# cereal yield and land scalar
cereal_scaler_v7.1 <- readr::read_csv(file.path(rawDataFolder, "inst/extdata/mappings/GCAM7.1", "GCAM_Yield_R_Cereals_Scaler_Y.csv"),
                                      comment = "#"
)
use_data(cereal_scaler_v7.1, overwrite = T)

# food pcal, mt, kg conversion
food_cal_mt_kg_conversion_v7.1 <- readr::read_csv(file.path(rawDataFolder, "inst/extdata/mappings/GCAM7.1", "food_cal_mt_kg_conversion.csv"),
                                                  comment = "#"
)
use_data(food_cal_mt_kg_conversion_v7.1, overwrite = T)


# primary, secondary, final energy maps
primary_energy_map_v7.1 <- readr::read_csv(file.path(rawDataFolder, "inst/extdata/mappings/GCAM7.1", "primary_energy_map.csv"),
                                    comment = "#"
) %>% gather_map()
use_data(primary_energy_map_v7.1, overwrite = T)

production_map_v7.1 <- readr::read_csv(file.path(rawDataFolder, "inst/extdata/mappings/GCAM7.1", "production_map.csv"),
                                comment = "#"
) %>% gather_map()
use_data(production_map_v7.1, overwrite = T)

secondary_energy_map_v7.1 <- readr::read_csv(file.path(rawDataFolder, "inst/extdata/mappings/GCAM7.1", "capacity_map.csv"),
                              comment = "#"
) %>%
...
capacity_map_v7.1 <- readr::read_csv(file.path(rawDataFolder, "inst/extdata/mappings/GCAM7.1", "capacity_map.csv"),
                              comment = "#"
) %>%
  dplyr::filter(!grepl("cogen", technology)) %>%
  gather_map()
...
cf_gcam_v7.1 <- readr::read_csv(file.path(rawDataFolder, "inst/extdata/mappings/GCAM7.1", "A23.globaltech_capacity_factor.csv"),
                         comment = "#", na = ""
)
use_data(cf_gcam_v7.1, overwrite = T)

cf_rgn_v7.1 <- readr::read_csv(file.path(rawDataFolder, "inst/extdata/mappings/GCAM7.1", "L223.StubTechCapFactor_elec.csv"),
                        comment = "#", na = ""
)
use_data(cf_rgn_v7.1, overwrite = T)

final_energy_map_v7.1 <- readr::read_csv(file.path(rawDataFolder, "inst/extdata/mappings/GCAM7.1", "final_energy_map.csv"),
                                  comment = "#"
) %>% gather_map()
use_data(final_energy_map_v7.1, overwrite = T)

en_demand_price_map_v7.1 <- readr::read_csv(file.path(rawDataFolder, "inst/extdata/mappings/GCAM7.1", "en_demand_price_map.csv"),
                                  comment = "#"
)
...
energy_price_map_v7.1 <- readr::read_csv(file.path(rawDataFolder, "inst/extdata/mappings/GCAM7.1", "en_price_map.csv"),
                                   comment = "#", na = ""
) %>% gather_map()
use_data(energy_price_map_v7.1, overwrite = T)
...
buildings_en_service_v7.1 <- readr::read_csv(file.path(rawDataFolder, "inst/extdata/mappings/GCAM7.1", "buildings_en_service.csv"),
                                      comment = "#"
) %>% gather_map()
...
capital_gcam_v7.1 <- readr::read_csv(file.path(rawDataFolder, "inst/extdata/mappings/GCAM7.1", "L223.GlobalIntTechCapital_elec.csv"),
  ...
  dplyr::bind_rows(readr::read_csv(file.path(rawDataFolder, "inst/extdata/mappings/GCAM7.1", "L223.GlobalTechCapital_elec.csv"),
                            comment = "#", na = ""
  )) %>%
...
investment_v7.1 <- readr::read_csv(file.path(rawDataFolder, "inst/extdata/mappings/GCAM7.1", "investment.csv"),
                            na = ""
) %>%
...
nonco2_content_v7.1 <- readr::read_csv(file.path(rawDataFolder, "inst/extdata/mappings/GCAM7.1", "L201.ghg_res.csv"),
                                comment = "#", na = ""
)
use_data(nonco2_content_v7.1, overwrite = T)

iea_capacity_v7.1 <- readr::read_csv(file.path(rawDataFolder, "inst/extdata/mappings/GCAM7.1", "IEAWEO2019_Capacity.csv"))
use_data(iea_capacity_v7.1, overwrite = T)

co2_market_v7.1 <- readr::read_csv(file.path(rawDataFolder, "inst/extdata/mappings/GCAM7.1", "CO2market_new.csv"), comment = "#")
use_data(co2_market_v7.1, overwrite = T)

co2_market_frag_map_v7.1 <- readr::read_csv(file.path(rawDataFolder, "inst/extdata/mappings/GCAM7.1", "CO2market_frag_map.csv"),
                                     comment = "#"
)
use_data(co2_market_frag_map_v7.1, overwrite = T)

# iron and steel
iron_steel_trade_map_v7.1 <- readr::read_csv(file.path(rawDataFolder, "inst/extdata/mappings/GCAM7.1", "iron_steel_trade.csv"), comment = "#") %>% gather_map()
use_data(iron_steel_trade_map_v7.1, overwrite = T)

# water
water_map_v7.1 <- readr::read_csv(file.path(rawDataFolder, "inst/extdata/mappings/GCAM7.1", "water.csv"), comment = "#") %>% gather_map()
use_data(water_map_v7.1, overwrite = T)

conveyance.eff_v7.1 <- readr::read_csv(file.path(rawDataFolder, "inst/extdata/mappings/GCAM7.1", "conveyance.eff.csv"), comment = "#")
use_data(conveyance.eff_v7.1, overwrite = T)

# transport sales & stock
ucd_size_class_v7.1 <- readr::read_csv(file.path(rawDataFolder, "inst/extdata/mappings/GCAM7.1",
                                          "UCD_size_class_revisions.csv"), comment = "#")
use_data(ucd_size_class_v7.1, overwrite = T)

ucd_core_v7.1 <- readr::read_csv(file.path(rawDataFolder, "inst/extdata/mappings/GCAM7.1",
...
use_data(transport_stock_map_v7.1, overwrite = T)
transport_sales_map_v7.1 <- readr::read_csv(file.path(rawDataFolder, "inst/extdata/mappings/GCAM7.1",
                                               "trn_sales_map.csv"), comment = "#") %>% gather_map()
use_data(transport_sales_map_v7.1, overwrite = T)
