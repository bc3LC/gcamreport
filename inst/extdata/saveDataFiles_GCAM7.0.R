...
L100.AgMIP_FoodWaste_Share_Pathway_SSP_v7.0 <- readr::read_csv(file.path(rawDataFolder, "inst/extdata/mappings/GCAM7.0", "L100.AgMIP_FoodWaste_Share_Pathway_SSP.csv"),
                                                                            comment = "#"
) %>%
  dplyr::rename(ssp = scenario) %>%
  dplyr::select(-GCAM_region_ID)
use_data(L100.AgMIP_FoodWaste_Share_Pathway_SSP_v7.0, overwrite = T)

# wood fuel - industrial roundwood
WoodFuel_IndRoundwood_ratio_v7.0 <- readr::read_csv(file.path(rawDataFolder, "inst/extdata/mappings/GCAM7.0", "FAO_GCAM_Ratio_WoodFuel_IndRoundwood_R_Yh.csv"),
                                                    comment = "#"
)
use_data(WoodFuel_IndRoundwood_ratio_v7.0, overwrite = T)

# cereal yield and land scalar
cereal_scaler_v7.0 <- readr::read_csv(file.path(rawDataFolder, "inst/extdata/mappings/GCAM7.0", "GCAM_Yield_R_Cereals_Scaler_Y.csv"),
                                      comment = "#"
)
