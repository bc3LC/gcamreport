...
server <- function(input, output, session) {
  ...
  observeEvent(input$select_all_scen, {
    updateAwesomeCheckboxGroup(
      session = getDefaultReactiveDomain(), inputId = "selected_scen",
  ...
  observeEvent(input$select_all_years, {
    updateAwesomeCheckboxGroup(
      session = getDefaultReactiveDomain(), inputId = "selected_years",
      label = "Select years",
  ...
  observeEvent(input$select_all_cols, {
    updateAwesomeCheckboxGroup(
      session = getDefaultReactiveDomain(), inputId = "selected_cols",
  ...
  observeEvent(c(
    ...
    if (input$tab_box == "Data") {
      ...
      sel <- update_user_choices_plot(
        selected_scen = input$selected_scen,
      ...
      if (input$vars_grouping == "Grouped Variables") {
        ...
        errors <- check_user_choices_plot(
          vars = sel$vars,
          scen = sel$scen,
          years = sel$years,
          reg = sel$reg,
          ...
          if (input$reg_grouping == "Grouped Regions") {
            ...
            assign(
              ...
              ggplot2::ggplot(data = data_sample, ggplot2::aes(x = year, y = values, color = Scenario, linetype = Variable, group = interaction(Scenario, Region, Variable))) +
                ...
                ggplot2::theme_bw() +
                ggplot2::theme(
                  legend.text = ggplot2::element_text(size = 8), legend.title = ggplot2::element_text(size = 10),
          ...
          local({
            ...
            output[[paste0("download", my_i)]] <- downloadHandler(
              ...
              content = function(file) {
                ...
                # save plot
                ggplot2::ggsave(file,
                  plot = get(paste0("fig_", tt)), device = "png",
          ...
          for (i in 1:n) {
            local({
              ...
              if (input$reg_grouping == "Grouped Regions") {
                # consider regions in the legend
                assign(
                  paste0("fig_", unique(data_sample$Variable)[my_i]),
              ...
              output[[paste0("download", my_i)]] <- downloadHandler(
                filename = function() {
                  paste0("fig_", unique(data_sample$Variable)[my_i], ".png")
                },
                content = function(file) {
                  assign(
                    paste0("fig_", unique(data_sample$Variable)[my_i]),
                  ...
                  # save plot
                  ggplot2::ggsave(file,
                    plot = get(paste0("fig_", unique(data_sample$Variable)[my_i])), device = "png",
                    height = hh_dwn, width = 20 + w, units = "cm", limitsize = FALSE
                  )
                }
              )
  ...
  observeEvent(c(
    input, input$select_none_regions,
