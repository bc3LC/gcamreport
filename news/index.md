...
`gcamreport` package with a new structure!!!💥 Compatible with GCAM 6.0,
7.0, and 7.1!

Novelties:

- Compatibility with GCAM 7.1.
- New structure to maintain all the pkg capabilities regardless of the
  compatible GCAM-core version.
- Improved robustness so that the pkg indicates which mappings need to
  be modified.
...
- JOSS Publication.
- Allow `gcamreport` to be used without cloning the repository or using
  the .Rproj file. Now suitable for running on clustered machines.
- Allow to specify query files.
- Add default options for selecting database or rgcam project scenarios.
- Add tutorials for mapping modifications and query files
  specifications.
...
**Full Changelog**:
<https://github.com/bc3LC/gcamreport/compare/v7.0.0>…v7.0.1
...
- Updated mappings to meet the GCAM 7.0 novelties.  
- Allow selection of regions and variables.  
- Add user error checks.  
- Simplify the UI launch.  
- Rename functions to make them more intuitive.  
- Fix: Report all the calculated elements.  
...
- Introduce user messages.  
- Add a step-by-step tutorial and other tutorial examples.  
...
- Mapping update to meet the new GCAM version requirements (with
  detailed gas trading features).  
...
- Creation of a template for the desired project following the IPCC
  requirements.  
- Friendly UI to view and subset the data items in a table, as well as
  to plot the desired variables choosing the desired regions and
  scenarios.  
